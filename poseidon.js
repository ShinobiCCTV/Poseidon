...
var Poseidon = function () {
    function Poseidon(options, callback) {
        ...
        if (typeof callback === 'function' && callback.length === 2) {
            ...
            this._callback = function (err, msg) {
                if (err) {
                    console.error('Poseidon Error: ' + err + ' Namespace: ' + _this._namespace);
                    return;
                }
                console.log('Poseidon Message: ' + msg + ' Namespace: ' + _this._namespace);
            };
        }
    ...
    _createClass(Poseidon, [{
        ...
        value: function start() {
            ...
            this._playing = true;
            this._socket = io(this._monitor.url, { transports: ['websocket'], forceNew: false });
